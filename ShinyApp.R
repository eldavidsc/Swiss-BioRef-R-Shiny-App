...
#data <- readRDS("PotassiumData.rds")
#päth <- paste0(getwd(),"/Labtests_Subsets/Creatinine/CreatinineData.Rds")
#data <- readRDS(päth)
...
#sex <- c("f","m")
#age_show <- c(10,20,30,40,50,60,70,80)
...
#MT_corr_show <- c("FDR / qValue","Holm-Bonferroni")
...
server <- function(input, output) {
  ...
  #path <- reactive({if(v$clustering_active){"T:/Desktop/Data/Databeis/T2_Clustering/"}else{"T:/Desktop/Data/Databeis/T2/"}})
  ...
  globalminus <- reactive({globalminus_shiny2.1(slice(),ptable_FDR(),input$xminmax[1],input$xminmax[2],name())})
  ...
  diag_compare_density_plot <- reactive({diag_compare_density2(slice(),input$diag_inspect,input$bandwidth)}) 
